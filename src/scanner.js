...
const QUALIFIED_MODEL_RE = new RegExp(
  `\\b(openai|anthropic|google|gemini)[/:](${KNOWN_MODEL})\\b`,
  "gi",
);
const BEDROCK_ANTHROPIC_RE = /\banthropic\.(claude-[a-z0-9._:-]+)\b/gi;
const VERTEX_MODEL_RE = /\bpublishers\/(google|anthropic)\/models\/(gemini-[a-z0-9._:-]+|claude-[a-z0-9._:-]+)\b/gi;
...
function normalizeCandidate(value, providerHint) {
  ...
  if (qualified) {
    ...
    return `${provider}/${stripCloudSuffix(qualified[2].toLowerCase())}`;
  }
  ...
  if (!new RegExp(`^(?:${KNOWN_MODEL})$`, "i").test(candidate)) return null;
...
async function scanRepository(options = {}) {
  ...
  async function visit(absolutePath) {
    ...
    if (stat.isDirectory()) {
      if (relativePath !== "." && DEFAULT_IGNORED_DIRS.has(path.basename(absolutePath))) return;
